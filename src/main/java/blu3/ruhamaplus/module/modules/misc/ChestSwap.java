...
public class ChestSwap extends Module {
    ...
    public void onEnable() {
        ...
        {
            int l_Slot = FindChestItem(this.getSettings().get(2).toToggle().state);

            if (!this.getSettings().get(2).toToggle().state && l_Slot == -1)
            ...
            if (s != null && s.getItem() != Items.AIR)
            {
                if (s.getItem() instanceof ItemArmor)
                {
                    ...
                    if (armor.armorType == EntityEquipmentSlot.CHEST)
                    {
                        ...
                        final boolean cursed = this.getSettings().get(1).toToggle().state ? (EnchantmentHelper.hasBindingCurse(s)) : false;
