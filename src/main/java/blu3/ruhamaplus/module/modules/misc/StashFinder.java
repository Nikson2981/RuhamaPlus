...
public class StashFinder extends Module
{
    private static final List<SettingBase> settings = Arrays.asList(
            new SettingMode("Mode: ", "Current", "0, 0"),
            new SettingSlider(0.0D, 10.0D, 4.0D, 0, "Fly Gap: "),
            new SettingToggle(false, "Debug"),
            new SettingToggle(true, "AutoReopen"),
            new SettingToggle(true, "Shulker Log"),
            new SettingToggle(true, "Dupe Log"),
            new SettingToggle(true, "Chest Log"),
            new SettingToggle(true, "Sign Log"),
            new SettingToggle(false, "Illegal Log"),
            new SettingSlider(0.0D, 50.0D, 20.0D, 0, "Min Chest: "),
            new SettingToggle(true, "Active"),
            new SettingToggle(false, "Cacti"));
    ...
    public StashFinder()
    {
        super("StashFinder", 0, Category.MISC, "Explores Chunks around you using the elytra and logs stashes (saves in \".minecraft/blu3/ruhama+/\", /stashfinder x z to set a start point)", settings);
    }
    ...
    {
        if (this.getSetting(8).asToggle().state && packet instanceof SPacketChunkData && this.mc.world.getBiome(this.mc.player.getPosition()) != Biomes.HELL)
        {
            ...
            if (this.getSetting(11).asToggle().state){ blocks.add(Blocks.CACTUS); }
                    ...
                    for (int y = 0; y < 255; ++y) {
                        for (int z = 0; z < 15; ++z) {
                            ...
                            if (!(!blocks.contains(b) || !(b != Blocks.BEDROCK || y > 5))) {
                                this.mc.ingameGUI.getChatGUI().printChatMessage(new TextComponentString(ChatFormatting.DARK_AQUA + "Illegal: " + ChatFormatting.RESET + b.getLocalizedName() + ": " + (chunk.x * 16 + x) + ", " + y + ", " + (chunk.z * 16 + z)));
    ...
    public void onEnable()
    {
        ...
        if (this.getSetting(8).asToggle().state)
        {
    ...
    {
        if (this.getSetting(10).asToggle().state)
        {
            StringBuilder content;
            Iterator teIter;
            TileEntity t;

            if (this.getSetting(4).asToggle().state && this.mc.player.ticksExisted % 10 == 0)
            {
                    ...
                    while (true)
                    {
                        ...
                        do
                        {
                            do
                            {
                                ...
                                {
                                    this.mc.ingameGUI.getChatGUI().printChatMessage(new TextComponentString(ChatFormatting.DARK_AQUA + "Shulker: " + ChatFormatting.RESET + + t.getPos().getX() + ", " + t.getPos().getY() + ", " + t.getPos().getZ()));
            ...
            if (this.getSetting(5).asToggle().state && this.mc.player.ticksExisted % 10 == 0)
            {
                content = new StringBuilder();
                ...
                while (tileEntityIterator.hasNext())
                {
                    ...
                    if (!this.chestList.contains(e.getKey()) && (double) e.getValue() >= this.getSetting(9).asSlider().getValue())
                    {
                        this.chestList.add(e.getKey());
                        String text = e.getValue() + "x Chest | " + e.getKey().getXStart() + ", " + e.getKey().getZStart();

                        this.mc.ingameGUI.getChatGUI().printChatMessage(new TextComponentString(ChatFormatting.DARK_AQUA + text));
            ...
            if (this.getSetting(6).asToggle().state && this.mc.player.ticksExisted % 10 == 0)
            {
                content = new StringBuilder();
                ...
                while (teIter.hasNext())
                {
                    ...
                    if (e instanceof AbstractChestHorse)
                    {
                        ...
                        if (e1.hasChest() && !this.dupeList.contains(e1.getUniqueID()))
                        {
                            ...
                            this.mc.ingameGUI.getChatGUI().printChatMessage(new TextComponentString(ChatFormatting.DARK_AQUA + "Dupe / " + ChatFormatting.RESET + e1.getDisplayName().getUnformattedText() + ": " + e1.posX + ", " + e1.posY + ", " + e1.posZ));
            ...
            if (this.getSetting(7).asToggle().state && this.mc.player.ticksExisted % 10 == 0)
            {
                content = new StringBuilder();
                teIter = this.mc.world.loadedTileEntityList.iterator();
                ...
                while (teIter.hasNext())
                {
                    ...
                    if (t instanceof TileEntitySign)
                    {
                        ...
                        if (!this.signs.contains(t1.getPos()) && !Arrays.equals(t1.signText, new ITextComponent[] {new TextComponentString(""), new TextComponentString(""), new TextComponentString(""), new TextComponentString("")}))
                        {
                            ...
                            this.mc.ingameGUI.getChatGUI().printChatMessage(new TextComponentString(ChatFormatting.DARK_AQUA + "Sign: <\"" + ChatFormatting.RESET + t1.signText[0].getUnformattedText() + " | " + t1.signText[1].getUnformattedText() + " | " + t1.signText[2].getUnformattedText() + " | " + t1.signText[3].getUnformattedText() + "\"> at: " + t1.getPos().getX() + ", " + t1.getPos().getY() + ", " + t1.getPos().getZ()));
            ...
            boolean flat = Objects.requireNonNull(ModuleManager.getModuleByName("ElytraFly")).isToggled() && Objects.requireNonNull(ModuleManager.getModuleByName("ElytraFly")).getSetting(0).asMode().mode == 0;

            if (!flat && this.getSetting(3).asToggle().state && this.elytratime > 0 && !this.mc.player.onGround && this.mc.currentScreen != null) {
            ...
            if (flat && !this.mc.gameSettings.keyBindBack.isKeyDown())
            {
                ...
                if (!this.nextChunks.isEmpty())
                {
                    ...
                    Vec3d forward = (new Vec3d(0.0D, 0.0D, Objects.requireNonNull(ModuleManager.getModuleByName("ElytraFly")).getSetting(4).asSlider().getValue())).rotateYaw(-((float) Math.toRadians(this.mc.player.rotationYaw)));
                    this.mc.player.setVelocity(forward.x, forward.y, forward.z);
                } else
                {
                    this.chunks.clear();
                    this.range += (int) this.getSetting(1).asSlider().getValue();
